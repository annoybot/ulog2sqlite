...
fn insert_or_update_data(conn: &Connection, table_name: &str, timestamp: i64, field_list: Vec<(String, inst::BaseType)>) -> Result<(), Box<dyn Error>> {
...
// ColumnDef.  Used for creating a unique list of column sqlite column definitions.
#[derive(Debug, Clone)]
struct ColumnDef {
    name: String,
